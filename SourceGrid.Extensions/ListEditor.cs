    ...
    public class ListEditor : UserControl
    {
        ...
        private void LoadEditors()
        {
            if (m_ItemType != null)
            {
                ...
                m_Editors = new EditorBase[l_Tmp.Count];
                m_Properties = new PropertyInfo[l_Tmp.Count];

                for (int i = 0; i < l_Tmp.Count; i++)
                {
                    var prop = (PropertyInfo) l_Tmp[i];
                    m_Properties[i] = prop;
                    m_Editors[i] = Factory.Create(prop.PropertyType);
                    if (m_Editors[i] != null)
        ...
        public void LoadList()
        {
            ...
            for (int i = 0; i < m_Properties.Length; i++)
            {
                var l_Header = new ColumnHeader(m_Properties[i].Name);
...
//					l_Header.Comparer = new DisplayStringCellComparer();
        ...
        private void PopulateCell(int p_GridRow, int p_GridCol, PropertyInfo p_PropInfo, EditorBase p_Editor,
                                  object p_Object)
        {
            grid[p_GridRow, p_GridCol] = new Cell(p_PropInfo.GetValue(p_Object, null));
            grid[p_GridRow, p_GridCol].Editor = p_Editor;

            var l_Bind = new BindProperty(p_PropInfo, p_Object);
            grid[p_GridRow, p_GridCol].AddController(l_Bind);

            var l_CustomEvents = new CustomEvents();
            l_CustomEvents.ValueChanged += Grid_ValueChanged;
        ...
        private void btrRefreshList_Click(object sender, EventArgs e)
        {
            try
            {
                LoadList();
        ...
        {
            var resources = new System.Resources.ResourceManager(typeof (ListEditor));
            this.grid = new Grid();
