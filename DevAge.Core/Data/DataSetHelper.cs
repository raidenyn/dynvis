...
namespace DevAge.Data
{
    ...
    public class DataSetHelper
    {
        ...
        public static DataTable SelectDistinct(DataTable SourceTable, string FieldName, bool pAddEmptyValue,
                                               object EmptyValue)
        {
        ...
        public static DataTable SampleDataByInterval(DataView sourceView, string[] columns, string samplingField,
                                                     int intervalValue, DateTimeInterval intervalType)
        {
            ...
            if (sourceView.Count > 0)
            {
                ...
                for (int i = 1; i < sourceView.Count; i++)
                {
                    ...
                    var currentValue = (DateTime) dr[samplingField];
                    int currentInterval;
        ...
        private static readonly DataTable mEmptyDataTable = new DataTable();
        ...
        public static object EvalRowExpression(DataRow row, string expression)
        {
            ...
            for (int i = 0; i < row.Table.Columns.Count; i++)
            {
                ...
                matchString = Regex.Escape("[" + row.Table.Columns[i].ColumnName + "]");

                if (Regex.IsMatch(expToEval, matchString, RegexOptions.IgnoreCase))
                    expToEval = Regex.Replace(expToEval, matchString, ExpressionFormat(row[i]), RegexOptions.IgnoreCase);

                // \b match only whole word
                matchString = @"\b" + Regex.Escape(row.Table.Columns[i].ColumnName) + @"\b";
