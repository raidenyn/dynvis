...
namespace DevAge.Data.SqlClient
{
    ...
    public class SqlCommandBuilder
    {
        ...
        public DataColumn[] IdentityColumns { get; set; }
        ...
        protected virtual string CreateSqlUpdateColumn()
        {
            ...
            int pIndex = mPrimaryKeys.Length;
                //start the parameter count from the number of primary keys because I first populate the primary key columns
        ...
        protected virtual void PopulateAllParameters(SqlParameterCollection parameters)
        {
            ...
            for (int i = 0; i < mPrimaryKeys.Length; i++)
            {
                parameters.Add("@p" + pIndex, SqlDbTypeFromDataType(mPrimaryKeys[i].DataType), 0,
                               mPrimaryKeys[i].ColumnName);
                pIndex++;
            }
            ...
            for (int i = 0; i < mNormalColumns.Length; i++)
            {
                parameters.Add("@p" + pIndex, SqlDbTypeFromDataType(mNormalColumns[i].DataType), 0,
                               mNormalColumns[i].ColumnName);
                pIndex++;
            }
        }
        ...
        public virtual SqlCommand GetInsertCommand()
        {
            ...
            string sql = string.Format(cmdFormat, CreateSqlTableName(), CreateSqlInsertColumnName(),
                                       CreateSqlInsertParameters(command.Parameters));
        ...
        public virtual SqlCommand GetUpdateCommand()
        {
            ...
            command.CommandType = CommandType.Text;
            PopulateAllParameters(command.Parameters);
            string sql = string.Format(cmdFormat, CreateSqlTableName(), CreateSqlUpdateColumn(),
                                       CreateSqlPKWhereColumn());
            command.CommandText = sql;
