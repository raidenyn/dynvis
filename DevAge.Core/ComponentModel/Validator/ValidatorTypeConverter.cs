...
namespace DevAge.ComponentModel.Validator
{
    ...
    public class ValidatorTypeConverter : ValidatorBase
    {
        ...
        public ValidatorTypeConverter(Type p_Type, TypeConverter p_TypeConverter) : base(p_Type)
        {
        ...
        public override bool IsStringConversionSupported()
        {
            ...
            if (m_TypeConverter != null)
                return AllowStringConversion && m_TypeConverter.CanConvertFrom(typeof (string)) &&
                       m_TypeConverter.CanConvertTo(typeof (string));
            else
        ...
        protected override void OnConvertingObjectToValue(ConvertingObjectEventArgs e)
        {
            ...
            else if (e.Value is string) //è importante fare prima il caso stringa per gestire correttamente il null
            {
                ...
                if (IsNullString(tmp))
                    ...
                    //se la stringa non è nulla e il tipo di destinazione è sempre una string allora non faccio nessuna conversione
                {
                }
        ...
        protected override void OnConvertingValueToObject(ConvertingObjectEventArgs e)
        {
            ...
            {
                e.Value = m_TypeConverter.ConvertTo(EmptyTypeDescriptorContext.Empty, CultureInfo, e.Value,
                                                    e.DestinationType);
            }
        }
