...
namespace SourceGrid
{
    ...
    public class RangeData
    {
        ...
        [NonSerialized] private DataObject mClipboardDataObject;
        ...
        static RangeData()
        {
            ...
            DataFormats.GetFormat(RANGEDATA_FORMAT);
        }
        ...
        public void LoadData(GridVirtual sourceGrid, Range sourceRange, Position startDragPosition, CutMode cutMode)
        {
            ...
            for (int r = mSourceRange.Start.Row; r <= mSourceRange.End.Row; r++, arrayRow++)
            {
                ...
                {
                    var posCell = new Position(r, c);
                    ICellVirtual cell = sourceGrid.GetCell(posCell);
                    var cellContext = new CellContext(sourceGrid, posCell, cell);
                    if (cell != null && cell.Editor != null && cell.Editor.IsStringConversionSupported())
                        mSourceValues[arrayRow, arrayCol] =
                            cell.Editor.ValueToString(cell.Model.ValueModel.GetValue(cellContext));
                    else if (cell != null)
            ...
            mClipboardDataObject = new DataObject();
        ...
        public void WriteData(GridVirtual destinationGrid, Range destinationRange)
        {
            ...
            if (CutMode == CutMode.CutOnPaste && mSourceGrid != null)
            {
                for (int sr = mSourceRange.Start.Row; sr <= mSourceRange.End.Row; sr++)
                    ...
                    {
                        var pos = new Position(sr, sc);
                        ICellVirtual cell = mSourceGrid.GetCell(sr, sc);
                        var cellContext = new CellContext(mSourceGrid, pos, cell);
                        if (cell.Editor != null)
                            cell.Editor.ClearCell(cellContext);
                    }
            }
            ...
            for (int r = newRange.Start.Row; r <= newRange.End.Row; r++, arrayRow++)
            {
                ...
                {
                    var posCell = new Position(r, c);
                    ICellVirtual cell = destinationGrid.GetCell(posCell);
                    var cellContext = new CellContext(destinationGrid, posCell, cell);
        ...
        public Range FindDestinationRange(GridVirtual destinationGrid, Position dropDestination)
        {
            ...
            var destinationStart = new Position(dropDestination.Row + (mSourceRange.Start.Row - mStartDragPosition.Row),
                                                dropDestination.Column +
                                                (mSourceRange.Start.Column - mStartDragPosition.Column));
        ...
        public static void ClipboardSetData(RangeData rangeData)
        {
            ...
            Clipboard.SetDataObject(rangeData.mClipboardDataObject);
        }
        ...
        public static RangeData ClipboardGetData()
        {
            ...
            {
                if (dtObj.GetDataPresent(DataFormats.Text, true))
                {
                    var buffer = (string) dtObj.GetData(DataFormats.Text, true);
