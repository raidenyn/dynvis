...
namespace SourceGrid.Cells.Views
{
    ...
    public class Cell : ViewBase
    {
        ...
        public Cell()
        {
            ElementsDrawMode = ElementsDrawMode.Align;
        }
        ...
        {
            ElementImage = (IImage) p_Source.ElementImage.Clone();
        ...
        private IImage mElementImage = new DevAge.Drawing.VisualElements.Image();
        ...
        {
            if (ElementText is TextRenderer)
            {
                var elementText = (TextRenderer) ElementText;
                ...
                elementText.TextFormatFlags |= Utilities.ContentAligmentToTextFormatFlags(TextAlignment);
            }
            else if (ElementText is TextGDI)
            {
                var elementTextGDI = (TextGDI) ElementText;

                if (WordWrap)
                    elementTextGDI.StringFormat.FormatFlags = 0;
                else
        ...
        {
            ElementImage.AnchorArea = new AnchorArea(ImageAlignment, ImageStretch);

            //Read the image
            System.Drawing.Image img = null;
            var imgModel = (Models.IImage) context.Cell.Model.FindModel(typeof (Models.IImage));
            if (imgModel != null)
