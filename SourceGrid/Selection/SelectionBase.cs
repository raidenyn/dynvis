...
namespace SourceGrid.Selection
{
    ...
    public abstract class SelectionBase
    {
        ...
        {
            SelectionChangedEventEnabled = true;
        }
        #endregion
        ...
        public bool Focus(Position pCellToActivate, bool pResetSelection)
        {
            ...
            if (pCellToActivate != ActivePosition)
            {
                ...
                ICellVirtual newCellToFocus = Grid.GetCell(pCellToActivate);
                var newCellContext = new CellContext(Grid, pCellToActivate, newCellToFocus);
                var gotFocusEventArgs = new ChangeActivePositionEventArgs(ActivePosition, pCellToActivate);
                ...
                if (IsActivePositionValid())
                {
                    ...
                    ICellVirtual oldCellFocus = Grid.GetCell(ActivePosition);
                    var oldCellContext = new CellContext(Grid, ActivePosition, oldCellFocus);
                    var lostFocusEventArgs = new ChangeActivePositionEventArgs(ActivePosition, pCellToActivate);
                ...
                //Fire a change event
                var newFocusRegion = new RangeRegion(pCellToActivate);
        ...
        public bool FocusColumn(int column)
        {
            if (Grid.Columns.Count > column)
            {
                ...
                {
                    var newFocus = new Position(r, column);
        ...
        public bool FocusRow(int row)
        {
            if (Grid.Rows.Count > row)
            {
                ...
                {
                    var newFocus = new Position(row, c);
        ...
        private Position SearchForValidFocusPosition()
        {
            for (int r = Grid.FixedRows; r < Grid.Rows.Count; r++)
            {
                ...
                {
                    var startPosition = new Position(r, c);
        ...
        public bool CanReceiveFocus(Position position)
        {
            if (Grid.CompleteRange.Contains(position))
            {
                ...
                if (cell != null)
                {
                    var context = new CellContext(Grid, position, cell);
        ...
        protected virtual void OnCellGotFocus(ChangeActivePositionEventArgs e)
        {
            ...
            //N.B. E' importante impostare prima la variabile m_FocusCell e dopo chiamare l'evento OnEnter, altrimenti nel caso in cui la cella sia impostata in edit sul focus, l'eseguzione va in loop (cerca di fare l'edit ma per far questo и necessario avere il focus ...)
        ...
        protected virtual void OnCellLostFocus(ChangeActivePositionEventArgs e)
        {
            ...
            {
                var rowArgs = new RowCancelEventArgs(focusRow);
            ...
            {
                var columnArgs = new ColumnCancelEventArgs(focusColumn);
        ...
        public bool MoveActiveCell(Position start, int rowShift, int colShift)
        {
                   ...
                   currentRow >= 0 && currentCol >= 0)
            {
                ...
                //verifico che la posizione di partenza non coincida con quella di focus, altrimenti significa che ci stiamo spostando sulla stessa cella perchи usa un RowSpan/ColSpan
        ...
        private Color mBackColor = Color.FromArgb(75, Color.FromKnownColor(KnownColor.Highlight));
        private RectangleBorder mBorder = new RectangleBorder(new BorderLine(Color.Black, 2));
        ...
        {
            if (SelectionChanged != null && SelectionChangedEventEnabled)
        ...
        //Добавлено Ю.С. Нагаевым 08.12.2008
        /// <summary>
        /// Enable or disable rise of SelectionChanged event
        /// </summary>
        public bool SelectionChangedEventEnabled
        {
            get; set;
        }
