...
namespace SourceGrid
{
    ...
    public partial class Grid : GridVirtual
    {
        ...
        public ICell this[int row, int col]
        {
            get
            {
                ICell cell = DirectGetCell(new Position(row, col));
                if (cell != null)
                    return cell;
                ...
                var requestPos = new Position(row, col);
        ...
        private void DirectSetCell(Position position, ICell cell)
        {
        ...
        private ICell DirectGetCell(Position position)
        {
            if (OptimizeMode == CellOptimizeMode.ForRows)
            {
                GridRow row = Rows[position.Row];
            ...
            if (OptimizeMode == CellOptimizeMode.ForColumns)
            {
                GridColumn col = Columns[position.Column];
        ...
        {
            ICell tmp = DirectGetCell(new Position(row, col));
        ...
        public override Range PositionToCellRange(Position pPosition)
        {
            ...
            ICell l_Cell = this[pPosition.Row, pPosition.Column];
            if (l_Cell == null)
        ...
        /// </summary>
        private readonly RangeCollection mDrawnRange = new RangeCollection();
        ...
        protected override void OnSortingRangeRows(SortRangeRowsEventArgs e)
        {
            ...
            {
                var rowInfoToSort = new RowInfo[e.Range.End.Row - e.Range.Start.Row + 1];
                var cellKeys = new ICell[e.Range.End.Row - e.Range.Start.Row + 1];
            ...
            {
                var l_RangeSort = new ICell[e.Range.End.Row - e.Range.Start.Row + 1][];
                var l_CellsKeys = new ICell[e.Range.End.Row - e.Range.Start.Row + 1];
                ...
                if (e.Ascending)
                {
                    for (int r = e.Range.Start.Row; r <= e.Range.End.Row; r++)
                    {
                        ...
                        for (int c = e.Range.Start.Column; c <= e.Range.End.Column; c++)
                        {
                            ...
                            if (tmp != null && tmp.Grid != null && tmp.Range.Start.Row >= 0 &&
                                tmp.Range.Start.Column >= 0) //verifico che la cella sia valida
                                RemoveCell(tmp.Range.Start.Row, tmp.Range.Start.Column);
                                    //la rimuovo dalla posizione precedente
