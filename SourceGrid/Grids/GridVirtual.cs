...
namespace SourceGrid
{
    ...
    public abstract class GridVirtual : CustomScrollControl
    {
        ...
        /// </summary>
        [DefaultValue(false)]
        public bool AutoStretchColumnsToFitWidth { get; set; }
        ...
        /// </summary>
        [DefaultValue(false)]
        public bool AutoStretchRowsToFitHeight { get; set; }
        ...
        public virtual void CheckPositions()
        {
            ...
            var completeRegion = new RangeRegion(complete);
            if (
        ...
        public virtual Position PositionAtPoint(Point point)
        {
            ...
            var realPos = new Position(row.Value, col.Value);
        ...
        public virtual void ChangeDragCell(CellContext cell, DragEventArgs pDragEventArgs)
        {
            ...
            {
                if (mDragCellPosition.IsEmpty() == false)
                    Controller.OnDragLeave(new CellContext(this, mDragCellPosition, GetCell(mDragCellPosition)),
                                           pDragEventArgs);
        ...
        private SelectionBase mSelection;
        ...
        {
            var newMouseSelection = new Range(Selection.ActivePosition, p_Corner);
        ...
        public virtual void ProcessSpecialGridKey(KeyEventArgs e)
        {
            ...
            {
                var focusCellContext = new CellContext(this, Selection.ActivePosition);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (var grCache = new GraphicsCache(e.Graphics, e.ClipRectangle))
            {
        ...
        protected virtual void OnRangePaint(RangePaintEventArgs e)
        {
            ...
            GraphicsState state = e.GraphicsCache.Graphics.Save();
            ...
            try
            {
                ...
                for (int r = e.DrawingRange.Start.Row; r <= e.DrawingRange.End.Row; r++)
                {
                    ...
                    for (int c = e.DrawingRange.Start.Column; c <= e.DrawingRange.End.Column; c++)
                    {
                        ...
                        if (cell != null)
                        {
                            ...
                            {
                                var cellContext = new CellContext(this, position, cell);
                ...
                //Draw the decorators
                foreach (DecoratorBase dec in Decorators)
                {
        ...
        /// </summary>
        protected virtual void PaintCell(GraphicsCache graphics,
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            ICellVirtual mouseCell = GetCell(mousePosition);

            //Call MouseMove on the cell that receive tha MouseDown event
            if (MouseDownPosition.IsEmpty() == false)
            {
                ICellVirtual mouseDownCell = GetCell(MouseDownPosition);
                if (mouseDownCell != null)
                    Controller.OnMouseMove(new CellContext(this, MouseDownPosition, mouseDownCell), e);
            }
            else
                //se non ho nessuna cella attualmente che ha ricevuto un mousedown, l'evento di MouseMove viene segnalato sulla cella correntemente sotto il Mouse
            {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            {
                var focusCell = new CellContext(this, Selection.ActivePosition);
            ...
            {
                ICellVirtual cellMouseDown = GetCell(position);
                if (cellMouseDown != null)
                {
                    ChangeMouseDownCell(position, position);

                    //Cell.OnMouseDown
                    var cellContext = new CellContext(this, position, cellMouseDown);
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            Position clickPosition = PositionAtPoint(PointToClient(MousePosition));
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            if (Selection.ActivePosition.IsEmpty() == false)
            {
                ICellVirtual focusCell = GetCell(Selection.ActivePosition);
                if (focusCell != null)
        ...
        private readonly LinkedControlsList m_LinkedControls;
        ...
        public virtual void ArrangeLinkedControls()
        {
            ...
            foreach (LinkedControlValue linked in m_LinkedControls)
            {
                ...
                ICellVirtual cell = GetCell(linked.Position);
        ...
        public void SortRangeRows(Range p_Range,
                                  ...
                                  IComparer p_CellComparer)
        {
            var eventArgs = new SortRangeRowsEventArgs(p_Range, keyColumn, p_bAscending, p_CellComparer);
        ...
        {
            var l_Cells = new ICellVirtual[Columns.Count];
            for (int c = 0; c < Columns.Count; c++)
        ...
        {
            var l_Cells = new ICellVirtual[Rows.Count];
            for (int r = 0; r < Rows.Count; r++)
        ...
        private readonly ColumnsBase m_Columns;
        private readonly RowsBase m_Rows;
        private int m_FixedCols;
...
//      "How to: Create Mirrored Windows Forms and Controls"  http://msdn2.microsoft.com/en-us/library/xwbz5ws0.aspx
        ...
        public virtual void ClearValues(RangeRegion region)
        {
            ...
            {
                var cellContext = new CellContext(this, pos);
        ...
        private readonly DecoratorList mDecorators = new DecoratorList();
        ...
        private readonly ControllerContainer mController = new ControllerContainer();
